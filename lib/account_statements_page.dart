...
class _AccountStatementsPageState extends State<AccountStatementsPage>{
 String party='seller';String range='month';DateTimeRange? custom;final search=TextEditingController();
 (DateTime,DateTime) dates(){final n=DateTime.now(),d=DateTime(n.year,n.month,n.day);if(range=='custom'&&custom!=null)return(DateTime(custom!.start.year,custom!.start.month,custom!.start.day),DateTime(custom!.end.year,custom!.end.month,custom!.end.day).add(const Duration(days:1)));return switch(range){'today'=>(d,d.add(const Duration(days:1))),'week'=>(d.subtract(Duration(days:d.weekday-1)),d.add(const Duration(days:1))),'previous'=>(DateTime(n.year,n.month-1,1),DateTime(n.year,n.month,1)),_=>(DateTime(n.year,n.month,1),DateTime(n.year,n.month+1,1))};}
 Future<void> pickCustom()async{final n=DateTime.now(),r=await showDateRangePicker(context:context,firstDate:DateTime(n.year-3),lastDate:n,initialDateRange:custom??DateTimeRange(start:DateTime(n.year,n.month,1),end:n));if(r!=null)setState((){custom=r;range='custom';});}
