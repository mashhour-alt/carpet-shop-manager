...
enum ReportRange { today,yesterday,week,month,previousMonth }
...
class _OperatingReportsPageState extends State<OperatingReportsPage> {
  ReportRange range=ReportRange.today; final search=TextEditingController();
  late Future<(OperatingSummary,List<OperatingSaleRecord>)> data=_load();
  (DateTime,DateTime) _dates(){final n=DateTime.now();final d=DateTime(n.year,n.month,n.day);return switch(range){
    ...
    ReportRange.previousMonth=>(DateTime(n.year,n.month-1,1),DateTime(n.year,n.month,1)),};}
  Future<(OperatingSummary,List<OperatingSaleRecord>)> _load() async {final (from,to)=_dates();final a=await widget.repository.loadOperatingSummary(widget.membership.institutionId,from,to);final b=await widget.repository.loadOperatingSales(widget.membership.institutionId,from,to,search:search.text);return(a,b);}
  void reload()=>setState(()=>data=_load());
  @override void dispose(){search.dispose();super.dispose();}
  String money(double v)=>v.toStringAsFixed(2)+' ر.س';
  String label(ReportRange r)=>switch(r){ReportRange.today=>'اليوم',ReportRange.yesterday=>'أمس',ReportRange.week=>'هذا الأسبوع',ReportRange.month=>'هذا الشهر',ReportRange.previousMonth=>'الشهر السابق'};
  @override Widget build(BuildContext context)=>FutureBuilder<(OperatingSummary,List<OperatingSaleRecord>)>(future:data,builder:(context,s){
    if(!s.hasData)return const Center(child:CircularProgressIndicator());final summary=s.data!.$1;final sales=s.data!.$2;
    return ListView(padding:const EdgeInsets.all(16),children:[
      Row(children:[Expanded(child:DropdownButtonFormField<ReportRange>(initialValue:range,decoration:const InputDecoration(labelText:'الفترة'),items:ReportRange.values.map((r)=>DropdownMenuItem(value:r,child:Text(label(r)))).toList(),onChanged:(v){range=v!;reload();})),const SizedBox(width:8),IconButton(onPressed:reload,icon:const Icon(Icons.refresh))]),
      ...
      ...summary.payments.entries.where((e)=>e.value!=0).map((e)=>ListTile(dense:true,title:Text(_payment(e.key)),trailing:Text(money(e.value)))),
