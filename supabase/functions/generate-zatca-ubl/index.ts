...
function validate(i:any,lines:any[]){
 ...
 if(!eq(sum("gross_amount"),Number(i.line_extension_amount)))fail("Invoice line extension parity failed");
 if(!eq(sum("discount_amount"),Number(i.discount_amount)))fail("Invoice discount parity failed");
 if(!eq(sum("taxable_amount"),Number(i.tax_exclusive_amount)))fail("Invoice taxable parity failed");
...
function build(i:any,lines:any[]){
