...
const rejectUnsafeConfigValue = (input: string, field: string) => {
  if (!input || /[\r\n\0]/.test(input)) throw new Error(`CSR ${field} is required and must be one line`);
  if (input.length > 200) throw new Error(`CSR ${field} exceeds 200 characters`);
  return input;
};
...
export function validateZatcaCsrFields(input: ZatcaCsrFields) {
  const fields = {
    ...
    egsSerialNumber: rejectUnsafeConfigValue(value(input.egsSerialNumber), "EGS serial number"),
    organizationIdentifier: value(input.organizationIdentifier),
    organizationUnitName: rejectUnsafeConfigValue(value(input.organizationUnitName), "organization unit"),
    organizationName: rejectUnsafeConfigValue(value(input.organizationName), "organization name"),
  ...
  if (!/^1-[^|]+\|2-[^|]+\|3-[^|]+$/.test(fields.egsSerialNumber)) {
    throw new Error("CSR EGS serial number must use 1-provider|2-model|3-serial format");
  }
  if (!/^3\d{13}3$/.test(fields.organizationIdentifier)) {
    throw new Error("CSR organization identifier must be a 15-digit VAT number starting and ending with 3");
  }
  if (!/^[A-Z]{2}$/.test(fields.countryCode)) {
    throw new Error("CSR country code must be ISO 3166-1 alpha-2");
  }
  if (!/^[01]{4}$/.test(fields.invoiceTypeMap) || fields.invoiceTypeMap === "0000") {
    throw new Error("CSR invoice type map must be four 0/1 digits and cannot be 0000");
  }
  if (fields.organizationIdentifier[10] === "1" && !/^\d{10}$/.test(fields.organizationUnitName)) {
    throw new Error("VAT-group CSR organization unit must be the 10-digit member TIN");
  }
  return fields;
}
 ...
 */
export function buildZatcaCsrOpenSslConfig(input: ZatcaCsrFields) {
...
export async function generateZatcaCsr(
  ...
  const openSslConfig = buildZatcaCsrOpenSslConfig(input);
  ...
  if (!/^-----BEGIN CERTIFICATE REQUEST-----[\s\S]+-----END CERTIFICATE REQUEST-----\s*$/.test(result.csrPem)) {
  ...
  if (!result.keyReference || /BEGIN .*PRIVATE KEY/.test(result.keyReference)) {
    throw new Error("CSR provider must return an opaque non-exportable key reference");
  }
