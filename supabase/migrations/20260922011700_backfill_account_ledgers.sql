...
select s.institution_id,s.id,'payment',-s.paid_total,'','legacy','رصيد مدفوع سابق قبل كشف الحساب',i.created_by,s.created_at
...
select t.institution_id,t.driver_id,'payment',-t.amount,coalesce(t.payment_method::text,''),'legacy','سداد مشوار سابق','trip',t.id,i.created_by,coalesce(t.paid_at,t.created_at)
